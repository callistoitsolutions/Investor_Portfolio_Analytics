...
# --- DATA LOADING (Centralized with Error Handling) ---
@st.cache_data
def load_and_process_data():
    try:
        ...
        # If current_value still missing, calculate it from ROI if available
        if 'current_value' not in df.columns:
            if 'roi_percent' in df.columns and 'investment_amount' in df.columns:
                ...
                # Assume no change if no other info
                df['current_value'] = df['investment_amount']
            else:
                st.error("❌ Missing required columns: investment_amount or current_value")
                st.info(f"Available columns: {', '.join(df.columns)}")
                return pd.DataFrame()
        
        # Run Analytics
        df = roi.calculate_roi(df)
        df = risk.calculate_risk(df) 
...
if df.empty:
    st.warning("⚠️ No data available. Please upload data or check database connection.")
    st.stop()

# --- DEBUG INFO (Uncomment to see your actual database columns) ---
with st.sidebar.expander("🔍 Debug: Database Columns", expanded=False):
...
with tab1:
    ...
    with col4:
        risk_mode = client_df['risk_level'].mode()[0] if not client_df.empty else "N/A"
    ...
    col5.metric("📂 Total Assets", len(client_df), delta=None)
    col6.metric("💵 Total Profit/Loss", f"₹{total_profit:,.0f}", delta=f"{(total_profit/total_invested)*100:.1f}%" if total_invested > 0 else "0%")
    ...
    with c2:
        if 'investment_start_date' in client_df.columns:
            perf_line = px.line(
                ...
                y='roi_percent',  # ✅ FIXED: Changed from roi_percent_calculated
...
with tab2:
    ...
    scatter_fig = px.scatter(
        ...
        labels={'risk_score': 'Risk Score', 'roi_percent': 'ROI (%)'},  # ✅ FIXED
...
with tab3:
    st.markdown("<h2>📤 Upload New Portfolio Data</h2>", unsafe_allow_html=True)
    
    st.info("💡 Upload an Excel file (.xlsx) with investor portfolio data. Required columns: client_id, asset_type, investment_amount, current_value")
    ...
    if uploaded_file is not None:
        try:
            ...
            # PRE-PROCESSING FIX: Clean columns before inserting
            ...
            st.success(f"✅ Data validated! {len(new_df)} rows ready for processing.")
            ...
            col1.metric("Total Rows", len(new_df))
            col2.metric("Unique Clients", new_df['client_id'].nunique() if 'client_id' in new_df.columns else "N/A")
            col3.metric("Total Investment", f"₹{new_df['investment_amount'].sum():,.0f}" if 'investment_amount' in new_df.columns else "N/A")
            
            st.markdown("<br>", unsafe_allow_html=True)
            
            if st.button("💾 Process & Insert to Database", type="primary"):
                with st.spinner("🔄 Inserting to database..."):
                    try:
                        engine = create_engine(CONNECTION_STRING)
                        new_df.to_sql("portfolio_data", engine, if_exists="append", index=False)
                        st.success("🎉 Data inserted successfully!")
                        st.balloons()
                        st.info("ℹ️ Please refresh the page to see updated data in the dashboard.")
                        ...
                        st.warning("Please check your database connection and table structure.")
                        
        except Exception as e:
            st.error(f"❌ Error reading file: {e}")
            st.warning("Please ensure the file is a valid Excel (.xlsx) format with proper column names.")
